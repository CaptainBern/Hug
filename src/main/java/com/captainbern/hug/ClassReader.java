...
public class ClassReader {
    ...
    public ClassReader(byte[] b, int offset, int length) {
        ...
        try {
            ...
            for (int i = 0; i < attributeCount; i++) {
                ...
                this.attributes.add(new DefaultAttribute(inputStream, pool.get(index)));
            }
     ...
     * @param classSource
     */
    public ClassReader(String classSource) {
    ...
    public Class defineClass() {
        ...
        }.defineClass(this.getClassName().replace('/', '.'), this.toBytecode());
    }
}
